...
function render() {
  ...
  renderYoyChart(rows);
  renderHeatmap(rows);
  renderShareCharts(); // period-only — intentionally ignores Principal/Site/Channel/Category filters
  renderTopline(rows);
...
function renderShareCharts() {
  // Deliberately scoped by reporting period ONLY — ignores Principal/Site/
  // Channel/Category filters per spec ("connected only to the reporting period").
  const rows = periodRows(state.raw);
